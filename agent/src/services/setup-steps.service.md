...
1. **A failed setup reported the worktree ready.** `queue.worktree.ensure` logged the setup command's
   failure and sent `queue.worktree.ready` regardless. The queue went idle, took a plan, and every
   bullet after it failed an hour in on a missing `node_modules`, with the install's error only ever in
   the journal.
2. **A worktree kept the dependencies it was created with, forever.** A plan that added a dependency
   landed, and every later plan in the queue built on the `node_modules` from before it.
...
config names. The first step that fails fails the queue with the step's name and the tail of its
output. Before every bullet, a step with `rerunWhen` runs again when the content of any listed file
differs from what it was the last time the step succeeded.

**A machine with no repository** keeps its one setup command, with the same two properties: a failure
fails the queue, and the command re-runs before a bullet when any tracked lockfile (`git ls-files`
...
- **State is per worktree, by slug**, in `~/.bosun/setup-state/<slug>.json`, and removed with the
  queue. A worktree with no recorded state runs its steps once — which is what an upgraded machine
