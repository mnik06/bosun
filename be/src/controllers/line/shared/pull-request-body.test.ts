...
describe('pullRequestBody', () => {
	...
	// A gap a person accepted is part of the verdict the reviewer must see, with who
	// took it.
	it('lists accepted gaps with who accepted them', () => {
		const result = body({
			knownGaps: [{ code: 'AC-4', text: 'Undo works', reproduction: 'undo does nothing', acceptedBy: 'lead@bosun.test' }]
		});

		expect(result).toContain('## Known gaps');
		expect(result).toContain('undo does nothing');
		expect(result).toContain('lead@bosun.test');
	});

	it('shows the diff of every conflict an integration resolved', () => {
		const integration = {
			status: 'done',
			onto: 'main',
			ontoSha: 'abcdef1234',
			merged: true,
			checks: 'passed',
			regenerated: [{ name: 'migrations', files: ['be/drizzle-out/0014_x.sql'] }],
			resolved: [{ file: 'be/src/users.routes.ts', diff: '+ route two' }]
		} as Integration;
		const result = body({ integrations: [integration] });

		expect(result).toContain('be/drizzle-out/0014_x.sql');
		expect(result).toContain('+ route two');
