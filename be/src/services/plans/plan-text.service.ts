export function getPlanTextService() {
	...
	return {
		append(opts: { planId: string; delta: string }): void {
			buffers.set(opts.planId, `${buffers.get(opts.planId) ?? ''}${opts.delta}`);
		},

		take(planId: string): string | null {
		...
		drop(planId: string): void {
			buffers.delete(planId);
		}
	};
}

export type PlanTextService = ReturnType<typeof getPlanTextService>;
