import { generateUiTicket } from 'src/services/keys/key.service';
...
export function issueTicket(userId: string): { ticket: string; expiresAt: Date } {
	const now = Date.now();

	dropExpired(now);

	const ticket = generateUiTicket();
