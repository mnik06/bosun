export function getPendingPingsService() {
	const pending = new Map<string, { machineId: string; sentAt: number }>();

	return {
		record(opts: { commandId: string; machineId: string; sentAt: number }): void {
			pending.set(opts.commandId, { machineId: opts.machineId, sentAt: opts.sentAt });
		},

		resolve(opts: { commandId: string; machineId: string; at: number }): number | null {
...
export type PendingPingsService = ReturnType<typeof getPendingPingsService>;
