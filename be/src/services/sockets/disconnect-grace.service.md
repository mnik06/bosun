...
heartbeat failed the in-flight run and the plan with it, and stopped the work with *the machine went
...
`stallMachineBuilds` read the row it had just been failed as no longer running, concluded bosun did
not want it any more, and sent `exec.cancel` — killing a session that was mid-edit. One blip cost
the bullet, the work, and the plan.

So the close schedules instead of settling. `hello` cancels the window, and `stallMachineBuilds`
...
- **The settle re-checks the registry.** Cancellation on `hello` is not enough on its own: a socket
  can register and be slow to announce. `pauseMachineBuilds` returns when the machine holds a socket.
- **Timers are unref'd.** A settle a shutdown skips is one the next `hello` does instead.

## Failure modes

- **Backend killed hard.** No close handler runs, no window is scheduled, and builds keep a `running`
  ghost job until the agent reconnects to the new instance and `hello` settles them.
- **Socket that registers and never says `hello`.** The window is cancelled by nothing, but the
  settle sees a live socket and returns, so the build keeps its job `running` until that socket closes — at
